...
export const formatInputDataToRequest = (inputValues: IinputValues) => {
  ...
  const formattedValues = {
    ...
    spicyFilter: spicyFilterItems?.find((option) => option?.value === spicyFilter)?.label,
    menuTypeFilter: menuTypeFilterItems?.find((option) => option?.value === menuTypeFilter)?.label,
    conceptFilter: conceptFilterItems?.find((option) => option?.value === conceptFilter)?.label,
  };

  let result: string = `
    1. 냉장고에 있는 재료: ${formattedValues?.materials},
    2. 맵기 정도: ${formattedValues?.spicyFilter},
    3. 음식 종류:${formattedValues?.menuTypeFilter}, 
    4. 목적: ${formattedValues?.conceptFilter}, 
...
export const formatResult = (rowData: string): Array<{ title: string; desc: string }> => {
